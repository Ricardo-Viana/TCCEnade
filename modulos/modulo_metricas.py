...
from operacoes.media_por_curso import processa_respostas


def modulo_metricas(anos_list, cod_list, valores_na, tipo_questao, relacao_ano_grupo):
    metricas = pd.DataFrame()

    df_cinerotulo = pd.read_csv('cine_rotulo_enade.csv')
    ...
    metricas.to_csv(f"tabelas_criadas/{anos_list}{cod_list}metricas_todos_cursos.csv", index=False)
